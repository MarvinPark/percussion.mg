...
const SMARTSTORE_SUPPLIER = "스마트스토어";

type SmartstoreOrderProduct = {
...
function buildSmartstoreSku(order: SmartstoreOrderProduct) {
  const code = order.sellerProductCode.trim();
  if (code) return code.slice(0, 80);
  return `SS-${order.productOrderId.replace(/[^a-zA-Z0-9]/g, "").slice(-12) || "item"}`;
}

export async function createProductFromSmartstoreOrder(
  supabase: SupabaseClient,
  order: SmartstoreOrderProduct,
  ...
  const sku = buildSmartstoreSku(order);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      sku,
      product_name: order.productName.trim() || "스마트스토어 상품",
      model_name: modelName || sku,
      supplier: SMARTSTORE_SUPPLIER,
  ...
  if (error) {
    ...
    return { error: "스마트스토어 제품 자동 등록에 실패했습니다." };
  }
