...
import { createProductForSaleLink } from "@/app/(main)/products/actions";
import PriceInput from "@/components/price-input";
import type { SaleProductOption } from "@/types/sale";

type SaleProductCreateModalProps = {
...
export default function SaleProductCreateModal({
  initialModelName,
  onClose,
  onCreated,
}: SaleProductCreateModalProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createProductForSaleLink({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-zinc-200 bg-white p-5 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="sale_create_supplier" className={labelClass}>
              공급처 <span className="text-red-500">*</span>
            </label>
            <input
              id="sale_create_supplier"
              value={supplier}
          ...
          <div>
            <label htmlFor="sale_create_product_name" className={labelClass}>
              제품명 <span className="text-red-500">*</span>
            </label>
            <input
              id="sale_create_product_name"
              value={productName}
          ...
          <div>
            <label htmlFor="sale_create_model_name" className={labelClass}>
              모델명 <span className="text-red-500">*</span>
            </label>
            <input
              id="sale_create_model_name"
              value={modelName}
          ...
          <div>
            <label htmlFor="sale_create_sku" className={labelClass}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="sale_create_category" className={labelClass}>
                카테고리
              </label>
              <input
                id="sale_create_category"
                value={category}
            ...
            <div>
              <label htmlFor="sale_create_brand" className={labelClass}>
                브랜드
              </label>
              <input
                id="sale_create_brand"
                value={brand}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="sale_create_purchase_price" className={labelClass}>
                매입가 (원)
              </label>
              <PriceInput
                id="sale_create_purchase_price"
                min={0}
                value={purchasePrice}
            ...
            <div>
              <label htmlFor="sale_create_sale_price" className={labelClass}>
                소비자가 (원)
              </label>
              <PriceInput
                id="sale_create_sale_price"
                min={0}
                value={salePrice}
