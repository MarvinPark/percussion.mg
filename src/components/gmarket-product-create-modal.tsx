...
function defaultSku(item: GmarketImportPreviewItem) {
  if (item.sellerProductCode.trim()) {
    return item.sellerProductCode.trim().slice(0, 80);
  }
  return `GM-${item.productOrderId.replace(/[^a-zA-Z0-9]/g, "").slice(-12) || "item"}`;
}

function defaultSalePrice(item: GmarketImportPreviewItem) {
  if (item.quantity <= 0) return item.totalPaymentAmount;
  return Math.round(item.totalPaymentAmount / item.quantity);
}

export default function GmarketProductCreateModal({
  item,
  onClose,
  onCreated,
}: GmarketProductCreateModalProps) {
  const [productName, setProductName] = useState(item.productName);
  const [modelName, setModelName] = useState(
    item.productOption.trim() || item.productName,
  );
  const [sku, setSku] = useState(defaultSku(item));
  const [supplier, setSupplier] = useState("지마켓");
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createProductForGmarketLink({
      ...
      >
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            지마켓 주문 상품을 재고에 등록합니다.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="gmarket_product_name" className={labelClass}>
              제품명
            </label>
            <input
              id="gmarket_product_name"
              value={productName}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="gmarket_sku" className={labelClass}>
                SKU
              </label>
              <input
                id="gmarket_sku"
            ...
            <div>
              <label htmlFor="gmarket_model_name" className={labelClass}>
                모델명
              </label>
              <input
                id="gmarket_model_name"
                value={modelName}
          ...
          <div>
            <label htmlFor="gmarket_product_option" className={labelClass}>
              옵션
            </label>
            <input
              id="gmarket_product_option"
              value={productOption}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="gmarket_supplier" className={labelClass}>
                공급처
              </label>
              <input
                id="gmarket_supplier"
          ...
          <div>
            <label htmlFor="gmarket_purchase_price" className={labelClass}>
              매입가
            </label>
            <PriceInput
              id="gmarket_purchase_price"
              min={0}
              value={purchasePrice}
