...
import {
  loadStockInPageSize,
  saveStockInPageSize,
} from "@/lib/stock-in-list-preferences";
...
type StockInListPageClientProps = {
  userId: string;
  movements: StockMovementWithProduct[];
  canManage?: boolean;
};

export default function StockInListPageClient({
  userId,
  movements,
  canManage = true,
}: StockInListPageClientProps) {
  ...
  useEffect(() => {
    setPageSize(loadStockInPageSize(userId));
    setRowFontSize(loadTableRowFontSize("stock-in", userId));
    setPreferencesLoaded(true);
  }, [userId]);

  useEffect(() => {
    if (!preferencesLoaded) return;
    saveStockInPageSize(userId, pageSize);
  }, [preferencesLoaded, pageSize, userId]);

  useEffect(() => {
    if (!preferencesLoaded) return;
    saveTableRowFontSize("stock-in", userId, rowFontSize);
