...
import { GMARKET_SCHEMA_SQL } from "@/lib/gmarket-schema-sql";
import type { SaleProductOption } from "@/types/sale";

type GmarketImportPanelProps = {
  canImport: boolean;
...
function summarizeResult(result: GmarketImportResult) {
  return [
...
export default function GmarketImportPanel({
  canImport,
  products,
}: GmarketImportPanelProps) {
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const [items, setItems] = useState<GmarketImportPreviewItem[]>([]);
  ...
  const [createModalItem, setCreateModalItem] =
    useState<GmarketImportPreviewItem | null>(null);
  ...
  useEffect(() => {
    ...
    getGmarketDefaultRange().then((range) => {
  ...
  function handlePreview() {
    ...
    startPreviewTransition(async () => {
      const result = await previewGmarketOrders(fromDate, toDate);
      if ("error" in result) {
      ...
      if (result.items.length === 0) {
        setMessage("해당 기간에 결제 완료된 지마켓 주문이 없습니다.");
      }
    });
  }
  ...
  function handleImport() {
    ...
    startImportTransition(async () => {
      const result = await importGmarketOrders(fromDate, toDate, {
        autoCreateProducts,
        manualMatches,
      });
      if ("error" in result) {
      ...
      setMessage(`지마켓 주문을 가져왔습니다. ${summarizeResult(result)}`);
  ...
  function handleCopySql() {
    void navigator.clipboard.writeText(GMARKET_SCHEMA_SQL).then(() => {
      setCopiedSql(true);
  ...
  return (
    <div className="rounded-xl border border-sky-200 bg-sky-50/70 dark:border-sky-900 dark:bg-sky-950/30">
      <div className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div>
          ...
          <p className="mt-1 text-xs text-sky-800/80 dark:text-sky-200/80">
            ESM API로 결제 완료 주문을 매출로 가져옵니다. API 호출 제한(5초당 1회)으로 조회에 시간이 걸릴 수 있습니다.
          </p>
        </div>
        <button
          type="button"
      ...
      {isOpen ? (
        <div className="space-y-4 border-t border-sky-200 px-4 py-4 dark:border-sky-900">
          <div className="flex flex-wrap items-end gap-3">
            ...
            {isPreviewPending ? (
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                지마켓 API는 5초당 1회 호출 제한이 있어 조회에 20초 이상 걸릴 수 있습니다.
              </p>
            ) : null}
            <button
              type="button"
          ...
          <label className="flex items-start gap-2 text-sm text-zinc-700 dark:text-zinc-300">
            ...
            />
            <span>
              미매칭 제품을 재고에 자동 등록 (공급처: 지마켓, 재고 0)
