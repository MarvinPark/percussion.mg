...
import { SALE_PRODUCT_OPTION_SELECT, type SaleProductOption } from "@/types/sale";
...
export async function createProductForSaleLink(input: {
  ...
  const result = await createInlineProduct({
    ...input,
    stock_quantity: 0,
  });
  ...
  return { product: toSaleProductOption(result.product) };
}
